...
func (o *GetEntryURL) Build() (*url.URL, error) {
	...
	var _path = "/entries/{entry_id}"

	entryID := o.EntryID
	if entryID != "" {
		_path = strings.Replace(_path, "{entry_id}", entryID, -1)
	} else {
		return nil, errors.New("entryId is required on GetEntryURL")
	}
...
func (o *GetEntryURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on GetEntryURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on GetEntryURL")
	}
